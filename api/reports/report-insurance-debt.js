...
router.post("/agent", function (req, res) {
    ...
    let insurance_typeFk = '';
    if (insurance_type_fk) {
        insurance_typeFk = `AND insurance_type_fk='${insurance_type_fk}'`;
    }
    let option_idFk = '';
    if (option_id_fk) {
    ...
    const conditions = `${datePays} ${companyId_fk} ${agentId_fk} ${insurance_typeFk} ${option_idFk}`;
...
router.post("/customer", function (req, res) {
    ...
    const tables = `view_insurance_all`;
    const fields = `*`;
    const wheres = `contract_status='1' AND custom_id_fk='${custom_id_fk}' AND status_company='${status_pay}'  ${conditions}`;
    const fieldFile = `*`;
    db.selectWhere(tables, fields, wheres, (err, results) => {
        if (err) {
            return res.status(400).send();
